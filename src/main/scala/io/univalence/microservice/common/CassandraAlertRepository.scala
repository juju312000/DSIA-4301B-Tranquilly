...
// Pas sur du type Array
  ...
  override def findPosition(idEnfant: Array): Iterator[Alert] = {
    val statement =
      session.prepare("SELECT * FROM tranquily.alert WHERE reason = 'TRACKING' AND idEnfant in ? ORDER BY timestamp DESC LIMIT 1")
